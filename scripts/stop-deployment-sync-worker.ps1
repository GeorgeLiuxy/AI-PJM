...
$statusFile = Join-Path $RuntimeRoot "deployment-sync-worker\status.json"
...
if (Test-Path -LiteralPath $statusFile) {
    $status = @{
        state = "stopped"
        message = "Deployment sync worker process stopped."
        updated_at = (Get-Date).ToUniversalTime().ToString("o")
    }
    $status | ConvertTo-Json | Set-Content -Path $statusFile
}

Write-Host "Deployment sync worker stopped."
