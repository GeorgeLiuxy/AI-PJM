...
$statusFile = Join-Path $RuntimeRoot "observability-alert-worker\status.json"
...
if (Test-Path -LiteralPath $statusFile) {
    $status = @{
        state = "stopped"
        message = "Observability alert worker process stopped."
        updated_at = (Get-Date).ToUniversalTime().ToString("o")
    }
    $status | ConvertTo-Json | Set-Content -Path $statusFile
}

Write-Host "Observability alert worker stopped."
