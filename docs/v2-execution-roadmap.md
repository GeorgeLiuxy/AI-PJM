...
生产级落地、上线门槛、权限安全、运维和团队试点计划以 `docs/production-readiness-plan.md` 为准。本文只跟踪交付链路功能状态，避免把 MVP 功能进度和生产化要求混在一起。
...
当前是“本地 MVP 闭环”，不是完整生产级系统。生产级缺口包括企业 SSO、业务角色细化、Vault/KMS 和密钥轮换、生产数据库、后台 Worker、真实 GitLab/GitHub、真实部署 Provider、审计报表和监控。
...
2. 收尾 P1：细化业务角色、补齐审计报表聚合，并准备企业 SSO 接入。
3. 完善 SecretStore。当前已补执行日志/执行证据脱敏和密钥健康检查；下一步让 GitLab/Dify/OpenAI/部署 Provider 按项目安全读取凭证，并补齐 Vault/KMS 和集中轮换策略。
...
原因：生产使用时最大的风险不是页面缺少某个按钮，而是权限、密钥、执行隔离、状态一致性和证据链不可靠。先补这些基础，平台才能真实减少人工搬运并稳定提升效率。
