...
当前状态：已完成本地首版。已有 `SecretStore` 服务端接口、项目级密钥表、Fernet 加密存储、密钥掩码响应、创建/轮换审计事件、访问管理页配置入口和基础权限测试。Dify Provider 会优先按项目读取 `dify_api_key`，未配置项目密钥时回退到全局 `DIFY_API_KEY`。主密钥通过 `SECRET_STORE_MASTER_KEY` 注入，未配置时禁止写入密钥。执行日志、执行证据和自测门禁证据已在持久化前进行敏感信息脱敏。
