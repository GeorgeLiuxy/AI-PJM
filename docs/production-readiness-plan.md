...
- 阻塞意见进入自动修复或人工处理。（状态、门禁回写和自动修复串联首版已完成；修复后更新原 MR 待实现）
- 支持 webhook，减少轮询。
